...
@php
    ...
    if (isset($locations)) {
        $dashboardLocations = $locations->map(function ($item) {
            return [
                'id' => $item->id,
                'title' => $item->title,
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    dashboardLocations.forEach(function (item) {
        ...
        marker.bindPopup(`
            <div style="min-width:250px">
                ...
                <div style="margin-top:6px">🕒 ${item.created ?? '-'}</div>
                <div style="margin-top:6px;color:#666">Lat: ${item.lat}<br>Lng: ${item.lng}</div>
            </div>
        `);
    ...
    if (bounds.length > 0) {
        ...
        L.popup().setLatLng([-6.2088, 106.8456]).setContent('Belum ada data lokasi').openOn(map);
    }
