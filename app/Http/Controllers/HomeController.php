    ...
    public function index()
    {
        ...
        */
        ...
        if ($user->role === 'admin_dept') {
            $dailyLocationQuery->whereHas(
                'routine.user',
                fn($q) => ($user->departemen_id && $user->divisi_id)
        ...
        */
        $dashboardLocations = collect()
            ...
            ->concat($dailyHistoryLocations->toBase()->map(fn($item) => [
            ...
            ->concat($taskLocations->toBase()->map(fn($item) => [
                'id'        => 'task_' . $item->id,
